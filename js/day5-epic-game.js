...
FIXED BUILD (duplicate declarations removed)
DAY 5 → DAY 6 VIDEO
=========================================================*/
...
// Portal to Day 6 - single source of truth (previously declared twice)
const PORTAL = {
    active: false,
    entered: false,
    x: 0,
...
on(musicSlider, "input", (e) => {
    if (bgMusic) bgMusic.volume = e.target.value / 100;
});

on(effectsSlider, "input", (e) => {
    AUDIOSETTINGS.effects = e.target.value / 100;
});

on(screenShakeToggle, "change", (e) => {
    AUDIOSETTINGS.screenShake = e.target.checked;
});
...
function updateGhostHealthBar(ghost) {
    ...
    if (fill) {
        fill.style.width = Math.max(0, (ghost.health / ghost.maxHealth) * 100) + "%";
    }
}
...
function killGhost(ghost) {
    ...
    if (scoreCounter) {
        scoreCounter.textContent = GAME.score.toString().padStart(6, "0");
    }

    if (aliveGhostsLabel) {
        aliveGhostsLabel.textContent = GHOSTS.filter(g => g.alive).length;
    }
...
function lightLantern(data) {
    ...
    if (lanternCounter) {
        lanternCounter.textContent = `${GAME.lanterns} / ${CONFIG.TOTAL_LANTERNS}`;
    }

    if (activeLanternsLabel) {
        activeLanternsLabel.textContent = GAME.lanterns;
    }
    ...
    setTimeout(() => { data.element.style.display = "none"; }, 550);
...
function updateLanternProximity() {
    ...
    if (interactionPrompt) {
        ...
        // Only auto-show the lantern prompt if the portal proximity check
        // (later in the frame) doesn't need to claim the prompt instead.
        if (nearAny) {
            interactionPrompt.style.display = "flex";
            interactionPrompt.textContent = "PRESS F TO LIGHT LANTERN";
        } else if (!PORTAL.active || !day6Portal || !day6Portal.classList.contains("inRange")) {
...
function tryInteract() {
    ...
    const nearLantern = LANTERNS.find(
        l => !l.lit && Math.hypot(PLAYER.x - l.x, PLAYER.y - l.y) < CONFIG.LANTERN_HINT_RADIUS
    );

    if (nearLantern) {
        lightLantern(nearLantern);
        return;
    }
    ...
    if (PORTAL.active && !PORTAL.entered) {
        ...
        if (dist <= CONFIG.PORTAL_HINT_RADIUS) {
            goToDay6();
        }
    }
}
...
function damageBossIfInRange() {
    ...
    if (BOSS.phase === 1 && BOSS.health < 650) {
        ...
        if (bossHealthFill) {
            bossHealthFill.style.background = "linear-gradient(90deg,#c40000,#ff6a00)";
        }
    ...
    if (BOSS.phase === 2 && BOSS.health < 300) {
        ...
        if (bossHealthFill) {
            bossHealthFill.style.background = "linear-gradient(90deg,#8b00ff,#ff2f6c)";
        }
...
function bossDefeated() {
    ...
    if (bossEl) {
        ...
        bossEl.style.transform = "translateY(-50px) scale(1.2)";
    ...
    // Portal opens shortly after the boss dies.
    setTimeout(() => {
        activatePortal(BOSS.x, BOSS.y);
    }, 1000);

    // Victory screen appears after the portal opens.
    setTimeout(() => {
        winGame();
    }, 2200);
}
...
function activatePortal(x, y) {
    ...
    day6Portal.style.transform = "translate(-50%, -60%) scale(.2)";
    day6Portal.style.transition = "opacity .8s ease, transform .8s ease";
    ...
    setTimeout(() => {
        showNotification("Approach the portal and press F, or hit CONTINUE!");
    }, 1200);
}
...
function goToDay6() {
    // Prevent double-clicks / multiple redirects.
    ...
    // Save completion.
    localStorage.setItem("day5Complete", "true");
    localStorage.setItem("day5Score", String(GAME.score));
    ...
    if (day6Portal) {
        ...
        day6Portal.style.opacity = "0";
        day6Portal.style.transform = "translate(-50%, -60%) scale(1.8)";
    }

    // Cinematic black transition.
    const transition = document.createElement("div");
    transition.id = "day6Transition";
    transition.style.position = "fixed";
    transition.style.inset = "0";
    transition.style.width = "100%";
    transition.style.height = "100%";
    transition.style.background = "#000";
    transition.style.zIndex = "999999";
    ...
    setTimeout(() => {
        window.location.href = CONFIG.NEXT_LEVEL_URL;
    }, 1100);
}
...
on(day6Portal, "click", (e) => {
    e.preventDefault();

    if (GAME.victory) {
        goToDay6();
    } else {
        showNotification("Defeat the Ghost King first!");
    }
});

on(day6Portal, "touchstart", (e) => {
    e.preventDefault();
...
/*=========================================================
PORTAL PROXIMITY
(No automatic teleport - the player must press F near the
portal or click CONTINUE on the victory screen.)
=========================================================*/
...
function updatePortalProximity() {
    ...
    const inRange = distance <= CONFIG.PORTAL_HINT_RADIUS;

    if (day6Portal) {
        day6Portal.classList.toggle("inRange", inRange);
    }

    if (interactionPrompt) {
        if (inRange && GAME.victory) {
            interactionPrompt.style.display = "flex";
            interactionPrompt.textContent = "PRESS F TO ENTER PORTAL";
        } else if (!LANTERNS.some(l => !l.lit && l.inRange)) {
            interactionPrompt.style.display = "none";
        }
    }
}
...
function updateCompass() {
    ...
    if (!target && BOSS.active && !BOSS.defeated) {
        target = BOSS;
    }
...
function winGame() {
    ...
    if (victoryScreen) {
        ...
        victoryScreen.style.transition = "opacity .6s ease";
...
on(quitEventButton, "click", () => { window.location.href = "index.html"; });
...
function initializeGame() {
    ...
    on(closeInventory, "click", () => {
        if (inventoryPanel) inventoryPanel.classList.remove("active");
    });

    if (lanternCounter) {
        lanternCounter.textContent = `0 / ${CONFIG.TOTAL_LANTERNS}`;
    }
